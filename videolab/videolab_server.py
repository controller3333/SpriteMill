...
# きつねの背景が5フレームで213→181と32ポイント動いていた通りドリフトは実在
# するので、この余裕は崖。inspect_walk_mp4.adaptive_magenta_thr がキャラ本体の
# 上端 (ハローを除くため収縮してから p99.5) を測り、危険帯 (thr-20) に入って
# いるときだけ (top+200)/2 へ寄せる (上限190)。実測13本で動いたのは
# らいよんちゃん1本 (70→134) のみ、他12本は据え置き = 平時の挙動は不変。
# 衝突していないキャラで動かさないのは、引き上げるとハロー帯が不透明で残り、
# デスピルの無い経路 (GPU側 _wp_key) で縁がマゼンタへ寄るため。
# ★これは「髪が溶ける」事故の対症ではない: 実測では thr=70 でも髪の**内部**の
# 喪失は0.00%で、1.67%は抜けて当然のアンチエイリアス縁だった。報告された
# 「完成シートで髪が真っ白」の原因は未特定のまま (この段では再現しない)。
# 測定を粗く (::4 + 収縮2回) すると余裕のあるロップまで70→127と誤判定した
# ので、全解像度・収縮3回で測ること (5.1ms/回 @2400x432)。
# 0.11.81: 8方向の割り当てを三段判定へ + 立ち絵の背景をマゼンタ地へ戻す。
# 0.11.81: 8方向の割り当てを三段判定へ + 立ち絵の背景をマゼンタ地へ戻す。
#
# ①方向 (ユーザー報告「完成物が正面を正面として取得できる確率がすごく低い」)。
# らいよんちゃん実走のペアキャンバスを実測したところ front/back・
# front_left/back_right・front_right/back_left の3組すべてが入れ替わっており
# (瞳の画素数 左0 / 右240-350)、一様な180°回転だった。0.11.76 の総当たり化は
# 「正面1枚を当てる→回り方を当てる」の直列推定は潰したが、**評価軸が C17
# 指示書=別キャラとの色差だけ**という弱点を据え置いていた (同じ弱点は
# engine/pipeline.py の _facing_margin_vs_poseset に既知として書かれていた)。
# 本番のC17指示書 (engine/facing_ref_*.png) で32通り総当たりした実測では
# 旧方式は**起点0/32・両方0/32** = 当たっていたのは偶然。誤りは独立3種で
# 効く軸も別なので三段に分けた:
#   ① 顔らしさ (上部45%の暗部密度×内部エッジ量) で180°を除外
#   ② 左右の鏡像整合で起点を決める (顔だけでは45°ずれが残る / 鏡像だけでは
#      180°を見抜けない — 入れ替わっても鏡像関係は保たれるため)
#   ③ facing_margin (自分 vs 自分の鏡像) で回りを決める。前後半球のスロット
#      集合も鏡像ペアも左右回りで同一なので①②には原理的に決められず、素の
#      色差も本番指示書では16/32=コイン投げだった
# 実測は起点32/32・回り32/32・両方32/32。回帰テストは合成画像では歯が立たな
# かった (旧方式でも16/16通る) ので、実キャラ2体×8方向を縮小した固定資産
# tests/fixtures/turntable/ を同梱してある (旧方式はこれで0/32)。
#
# ②背景 (2026-07-24ユーザー裁定)。0.11.52 で中立グレーへ逃げていたが、灰地は
# **閉じた背景穴を塗り替えられない** — key_to_magenta の種が画面の縁だけな
# ので、股下・脇下のようにキャラに囲まれた背景は縁と連結せず残る (髪も脚も
# 黒い輪郭線で囲まれているため外から入った塗り潰しが止まる)。実測で股下
# RGB(110,117,109) と髪の裾 RGB(110,117,105) が色距離4 = 背景色そのものが
# キャラの内側に居た。引っ張りは灰地でも起きているので色では防げず、違うのは
# 事故の重さだけ (灰=キー対象外だから残る / マゼンタ=対象だから消える)。
# マゼンタ地なら「ほぼ純マゼンタは確実に背景」が使えるので位置無制限の種を
# 足せる = 股下が塞がる。全損側は stills_ok の面積下限4% (事故当時の実測3.7%
# で較正済み) が回転段の手前で捕まえる。
# 加えて部分喪失 (体は残るが髪だけ溶ける) が既存ゲートの死角だったので
# gs.magenta_collision() で実測する — らいよんちゃんは危険帯40.3%・前景p98=69
# に対し他キャラは最大2.64%・p98最大54。**却下はしない** (桃色髪は依頼どおり
# の正しい絵で、種を変えても直らず3回無駄にして依頼を落とすだけ)。
# 髪色をネガティブで禁じるのも同じ理由で不可。
...
# (2-5、既定3)。
# 0.11.77: 足踏みの両端だけを立ち絵で錨止め (ユーザー報告「リファイン源
# 抜いたら、錨を入れないと周り出してます」)。refine源=静止動画だと動かず、
# 無錨だとAniSoraの回転priorで回る。sparse guidance (指定時刻だけ画像を
# 書き込み、間は自由) をストライド=フレーム数で使い、[0, 最終] の2枚だけ
# 立ち絵に固定する。中は自由に動け、一周ぶんの回転は溜められない。
...
# 設計上、正常な全身立ち絵も縦92-97%になるので割合では判定できない。
...
# 打ち消せない)。実出力が「静止コマ=マント無し/歩行コマ=全部マント」
# だったのは「動きで揺れる布」を忠実に描いた結果。"the cape" を削除し、
# 依頼に無い衣装名詞をプロンプト部品に書けないようlint testを追加。
...
# ①背景同系色の服をネガティブへ ②胴の中心線の残存率を実測するゲート
# (実走: 抜けた絵9% / 正常96-100%)。★面積だけの検査では通ってしまう。
# 0.11.55: 立ち絵プロンプトを「キャラ描写が先・技術タグが後」へ並べ替え。
# 定型句を積み増した0.11.54は顔なし・無彩色・後ろ向きを連発した (SDXLの
# CLIPは77トークンで切れ前方ほど強く効くため、依頼の言葉が押し出されて
# いた)。並べ替え後は同じ依頼が3/3で金髪・緑目・青チャイナの全身1体。
# 0.11.54: 立ち絵に必ず服を着せる。依頼の衣装が機械翻訳で服として
# 伝わらないと素体が出る (2026-07-23実走: palette「青チャイナ」→
# "blue china" で裸の立ち絵が生成され、回転段へ流れかけた)。衣装保証
# タグ+裸ネガティブを常時付与し、原文の衣装語 (チャイナ/巫女/セーラー等)
# を対訳表で英語タグへ補う。回転プロンプトにも脱衣禁止を追加。
# 0.11.53: 立ち絵が「5体並びのターンアラウンド表」になる事故を塞ぐ。
# 骨格を1体分しか渡していなくてもSDXLは縦長画面を複数体で埋める
# (2026-07-23実走・面積56%・4体は顔が空白)。solo系タグ+複数体ネガティブ
# と、ゲートに面積の上限 (45%) を追加。実走4枚: 正常21.8/22.7%、消失3.7%、
# 5体56.1% がこの窓できれいに分離する。
