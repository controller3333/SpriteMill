    ...
    def _gen_hemisphere(tag: str, layout, seed: int = 42) -> None:
        ...
        if _free:
            ...
            log(f"[{tag}] free_idle: idle/静止の骨格制御を撤去 "
                "(立ち絵の姿勢を尊重)")
        _pev = int(_exp.get("pose_every") or 0)
        if not _pev:
        ...
        if _pev > 1 and plan == "biped":
            ...
            frames = [fr if (k < idle_n or k > gait_end
                      ...
                      for k, fr in enumerate(frames)]
            log(f"[{tag}] 実験pose_every={_pev}: 歩行窓の骨格を間引き "
                "(中割はVACEに委ねる)")
