...
| `vace` | VACE-Fun（HighへAniSora要素LoRAを選択可） | A100 40GBはoffload必須 | OpenPose i2v | `vace_base=fun, vace_lora=anisora_high` でKijai LoRAをHighだけへ適用し、Lowは素のVACE |
| `vace_anisora_handoff` | VACE High + AniSora要素LoRA → VACE Low → native AniSora Low | A100 40GBはQ4推奨 | OpenPose i2v | 通常比較はAniSora Low終端1〜2step。`hybrid_lightning4=true, hybrid_vace_low_steps=1`でHigh2→VACE Low1→AniSora Low1 |
...
`refine_frames_b64` を使う再加工でも2枚目の `images_b64` を終端画像として
渡せる。25フレーム区間の始点/終点を隣接方向絵で共有する連鎖生成に使う。
通常の歩行パックは各方向57フレームで、f0--5を直立、f6--48を21フレーム
周期の歩行2周（周期境界を共有）、f49--56を直立静止にする。立ち絵計測が返す
`gait` と `limb_mode` を制御画像へ反映し、名前・コンセプトの走行語は
`gait=run` を強制する。`limb_mode=legs|arms|none` では存在しない部位の骨を
描かず、立ち絵と骨格の競合を避ける。
工房本線は各方向57フレームの採用外末尾4フレームを次方向への旋回に使い、
最終latent slotと `last_image` を隣接方向絵へ固定する。歩行2周期の採用範囲は
従来どおりで、旋回4フレームは方向間の受け渡し専用としてシートへ入れない。
方向別8本の生成後は共有フレームを重複させず、時計回りの連続中間成果
`canvas_AI_chain.mp4` も保存する。
...
以下のVACE系説明は、工房のレガシー再生成および通常GUIの比較経路。

- 2026-07-22時点の本命実験は `model=vace` に
  `{"vace_base":"fun","vace_lora":"anisora_high"}` を渡す。
  Kijai/WanVideo_comfy の
  `Wan2_2_I2V_AniSora_3_2_HIGH_rank_64_fp16.safetensors`（AniSora Highの
  要素を抽出したLoRA）だけをVACE Highへ装着し、VACE Lowは無改造のまま
  通常どおり仕上げる。Lowは非蒸留なので、隔離プローブの既定は30step / CFG 5。
- `vace_anisora_handoff` は比較用。上と同じKijai LoRAをVACE Highへ載せた後、
  native AniSora Lowへlatent直結するが、回転する力が強いため
  `hybrid_low_steps=1|2`（既定2）の終端だけに限定する。
- 三段4step比較は `hybrid_lightning4=true`, `hybrid_vace_low_steps=1`,
  `hybrid_low_steps=1`, `steps=4`, `guidance=1.0`。VACE High/Lowにだけ
  lightx2v Lightning LoRAを適用し、native AniSora Lowは最終stepだけ使う。
  安定重視の6stepは `hybrid_vace_low_steps=2`, `steps=6` とし、
  High 3→VACE Low 2→AniSora Low 1へ配分する。
...
- 比較用の `vace_anisora_handoff` は、選択された方向まとめキャンバス全体を
  **AniSora High要素LoRAつきVACE High → native AniSora Low終端1〜2step**
  の1軌道で生成する。中間MP4、
...
- 比較用handoffの既定は **49f / 16fps / 合計8step**（VACE High 6step＋
  AniSora Low終端2step）。
...
- AniSora Lowの担当数は `hybrid_low_steps=1|2`。境界値は旧依頼との互換入力として
  残るが、現行比較では担当step数を優先する。
