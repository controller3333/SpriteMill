...
class Figure:
    ...
    def __init__(self, head_frac: float = 0.40):
        ...
        self.hip_y = 0.56 * b
        # ---- 幅は頭身に**ゆるく**追従する (2026-07-23) ----
        # 旧実装は幅を完全な固定値にしていた ("頭身を変えても体の横幅は
        # 1pxも変わらない")。全高だけ 0.5/head_frac で伸びるので、頭身を
        # 上げるほど骨格は「同じ幅のまま縦に引き伸ばされた針金」になり、
        # ControlNet強度0.95でその形が絵にそのまま焼き付く — ユーザー報告
        # 「キャラクターが縦に伸びている」の実体がこれ (肩幅/全高の実測:
        # 2.5頭身 0.260 → 8頭身 0.081 と3.2倍細くなっていた。解剖学的な
        # 8頭身は0.17-0.19なので約半分)。
        # kw は校正アンカー (hf=0.40=2.5頭身) でちょうど1.0になる形にして、
        # チビ側の既存の見た目を1pxも動かさない。指数0.62は8頭身で
        # 肩幅/全高≒0.17へ着地する値 (0.0813*2.13≒0.173)。
        kw = min(2.2, max(1.0, (0.40 / hf) ** 0.62))
        self.width_scale = kw
        self.sh_x = 0.1625 * kw                   # 肩の半幅
        self.hip_x = 0.0625 * kw                  # 股関節の半幅 (広いと俯瞰の
        # 深度差で手前脚が沈み、遊脚クリアランスが画面上で相殺される)
        self.torso_r_top = 0.106 * kw
        self.torso_r_bot = 0.125 * kw
        self.arm_r = 0.040 * kw
        self.leg_r = 0.0525 * kw
        self.foot_len = 0.1125 * kw
        # ---- 高さ方向の寸法だけ体の縮尺に追従 ----
